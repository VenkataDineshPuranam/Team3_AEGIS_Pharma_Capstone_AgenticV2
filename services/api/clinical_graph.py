"""clinical_integrity LangGraph -- Stage 21. Same spine as services/api/graph.py's
batch_review graph, closing D03's OUT_OF_SCOPE injects (protocol-version divergence,
eligibility ambiguity, randomization-service outage, potential unblinding, eConsent
withdrawal mismatch, decentralized-device clock skew, endpoint-adjudication backlog,
site inspection risk) by building a real governed workflow rather than leaving the
domain unaddressed.
...
from packages.domain.payloads import ClinicalFinding, ClinicalPayload
...
from services.integration.clinical_integrity_check import ToolError as ReconcileError, integrity_check as tool_reconcile
...
CLINICAL_PRIMARY_APPROVER = "Clinical Trial Medical Monitor"


def build_clinical_graph(llm: LLMNodes | None = None, protocol_id: str = "CT-001", checkpointer=None):
    llm = llm or StubLLM()
    ...
    def retrieve(state: GovernedState) -> dict:
        try:
            result = tool_retrieve(
                run_id=state["run_id"], terms=["CLINICAL", "policy"],
    ...
    def reconcile(state: GovernedState) -> dict:
        ...
        try:
            result = tool_reconcile(
                run_id=state["run_id"], protocol_id=protocol_id,
                evidence_ids=evidence_ids, policy_contract_version=state["policy_contract_version"],
            )
        except ReconcileError:
            return {"terminal_state": "abstained", "abstention_reason": "dependency_unavailable"}
        findings = tuple(ClinicalFinding(**f) for f in result["findings"])
        payload = ClinicalPayload(protocol_id=protocol_id, reconciliation_complete=result["reconciliation_complete"], findings=findings)
        return {"domain_payload": payload, "tool_calls": state["tool_calls"] + 1}

    def synthesize(state: GovernedState) -> dict:
        evidence_ids = [e.evidence_id for e in state["evidence"]]
        cached = response_cache.get(state["workflow"], protocol_id, evidence_ids)
    ...
    def guard2_with_cache_write(state: GovernedState) -> dict:
        ...
        if result.get("guard_verdict") == "clear":
            evidence_ids = [e.evidence_id for e in state["evidence"]]
            response_cache.set_cleared(state["workflow"], protocol_id, evidence_ids, state["draft_output"])
        return result
    ...
    def hitl_route_node(state: GovernedState) -> dict:
        return {
            "hitl_required": True, "approver_roles": [CLINICAL_PRIMARY_APPROVER],
            "hitl_status": "pending", "hitl_tier": "T0",
        }

    def hitl_interrupt(state: GovernedState) -> dict:
        decision = hitl_decision.decode(interrupt(
            {"approver_roles": state["approver_roles"], "run_id": state["run_id"], "protocol_id": protocol_id}
        ))
    ...
    def finalize(state: GovernedState) -> dict:
        ...
        audit_store.write_agent_run(
            ...
            subject_id=protocol_id, requester_role=state["requester_role"],
