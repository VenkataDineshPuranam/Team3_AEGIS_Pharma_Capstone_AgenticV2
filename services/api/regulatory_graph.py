"""regulatory_completeness LangGraph -- Stage 21. Same spine as services/api/graph.py's
batch_review graph, closing D07's OUT_OF_SCOPE injects (IDMP identity conflict, labeling
divergence, commitment deadline ambiguity, eCTD sequence gap, variation classification
dispute, inspection request surge) by building a real governed workflow rather than
leaving the domain unaddressed.

Deliberately NOT sharing code with graph.py/pv_graph.py/supply_graph.py -- RR-2/T-10, same
reasoning as every other workflow graph's own module docstring.
"""
...
from packages.domain.payloads import RegulatoryFinding, RegulatoryPayload
...
from services.integration import audit_store, evidence_gate, prohibited_action_guard, response_cache
from services.integration.regulatory_completeness_check import ToolError as ReconcileError, completeness_check as tool_reconcile
...
REGULATORY_PRIMARY_APPROVER = "Head of Regulatory Affairs"


def build_regulatory_graph(llm: LLMNodes | None = None, submission_id: str = "REG-001", checkpointer=None):
    llm = llm or StubLLM()
    ...
    def retrieve(state: GovernedState) -> dict:
        try:
            result = tool_retrieve(
                run_id=state["run_id"], terms=["REGULATORY", "policy"],
    ...
    def reconcile(state: GovernedState) -> dict:
        ...
        try:
            result = tool_reconcile(
                run_id=state["run_id"], submission_id=submission_id,
                evidence_ids=evidence_ids, policy_contract_version=state["policy_contract_version"],
            )
        except ReconcileError:
            return {"terminal_state": "abstained", "abstention_reason": "dependency_unavailable"}
        findings = tuple(RegulatoryFinding(**f) for f in result["findings"])
        payload = RegulatoryPayload(submission_id=submission_id, reconciliation_complete=result["reconciliation_complete"], findings=findings)
        return {"domain_payload": payload, "tool_calls": state["tool_calls"] + 1}

    def synthesize(state: GovernedState) -> dict:
        evidence_ids = [e.evidence_id for e in state["evidence"]]
        cached = response_cache.get(state["workflow"], submission_id, evidence_ids)
        if cached is not None:
            return {"draft_output": cached}
        try:
    ...
    def guard2_with_cache_write(state: GovernedState) -> dict:
        ...
        if result.get("guard_verdict") == "clear":
            evidence_ids = [e.evidence_id for e in state["evidence"]]
            response_cache.set_cleared(state["workflow"], submission_id, evidence_ids, state["draft_output"])
        return result
    ...
    def hitl_route_node(state: GovernedState) -> dict:
        return {
            "hitl_required": True, "approver_roles": [REGULATORY_PRIMARY_APPROVER],
            "hitl_status": "pending", "hitl_tier": "T0",
        }

    def hitl_interrupt(state: GovernedState) -> dict:
        decision = hitl_decision.decode(interrupt(
            {"approver_roles": state["approver_roles"], "run_id": state["run_id"], "submission_id": submission_id}
        ))
    ...
    def finalize(state: GovernedState) -> dict:
        ...
        audit_store.write_agent_run(
            ...
            subject_id=submission_id, requester_role=state["requester_role"],
    ...
    graph.add_conditional_edges(
        "reconcile", lambda s: "abstain" if s.get("terminal_state") == "abstained" else "synthesize",
