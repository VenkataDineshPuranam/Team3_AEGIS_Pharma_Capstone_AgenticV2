"""research_review LangGraph -- Stage 21. Same spine as services/api/graph.py's
batch_review graph (langgraph_design.md SS7: "same spine, different middle"), closing
D02's OUT_OF_SCOPE injects (assay drift, compound genealogy collision, omics cohort bias,
preclinical image manipulation, unqualified research model, target-evidence conflict) by
building a real governed workflow rather than leaving the domain unaddressed.
...
from packages.domain.payloads import ResearchFinding, ResearchPayload
...
from services.integration.research_reconcile import ToolError as ReconcileError, reconcile as tool_reconcile
...
RESEARCH_PRIMARY_APPROVER = "Head of Preclinical Research"


def build_research_graph(llm: LLMNodes | None = None, research_id: str = "R-001", checkpointer=None):
    llm = llm or StubLLM()
    ...
    def retrieve(state: GovernedState) -> dict:
        try:
            result = tool_retrieve(
                run_id=state["run_id"], terms=["RESEARCH", "policy"],
    ...
    def reconcile(state: GovernedState) -> dict:
        ...
        try:
            result = tool_reconcile(
                run_id=state["run_id"], research_id=research_id,
                evidence_ids=evidence_ids, policy_contract_version=state["policy_contract_version"],
            )
        except ReconcileError:
            return {"terminal_state": "abstained", "abstention_reason": "dependency_unavailable"}
        findings = tuple(ResearchFinding(**f) for f in result["findings"])
        payload = ResearchPayload(research_id=research_id, reconciliation_complete=result["reconciliation_complete"], findings=findings)
        return {"domain_payload": payload, "tool_calls": state["tool_calls"] + 1}

    def synthesize(state: GovernedState) -> dict:
        evidence_ids = [e.evidence_id for e in state["evidence"]]
        cached = response_cache.get(state["workflow"], research_id, evidence_ids)
    ...
    def guard2_with_cache_write(state: GovernedState) -> dict:
        ...
        if result.get("guard_verdict") == "clear":
            evidence_ids = [e.evidence_id for e in state["evidence"]]
            response_cache.set_cleared(state["workflow"], research_id, evidence_ids, state["draft_output"])
        return result
    ...
    def hitl_route_node(state: GovernedState) -> dict:
        return {
            "hitl_required": True, "approver_roles": [RESEARCH_PRIMARY_APPROVER],
            "hitl_status": "pending", "hitl_tier": "T0",
        }

    def hitl_interrupt(state: GovernedState) -> dict:
        decision = hitl_decision.decode(interrupt(
            {"approver_roles": state["approver_roles"], "run_id": state["run_id"], "research_id": research_id}
        ))
    ...
    def finalize(state: GovernedState) -> dict:
        ...
        audit_store.write_agent_run(
            ...
            subject_id=research_id, requester_role=state["requester_role"],
