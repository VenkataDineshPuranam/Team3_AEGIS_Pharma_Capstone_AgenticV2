"""regulatory.completeness_check -- implements
packages/contracts/tool_contracts/regulatory_completeness_check.schema.json exactly, over
the synthetic regulatory fixtures (tests/fixtures/synthetic/regulatory/*.json). Stage 21's
...
FIXTURES_DIR = REPO_ROOT / "tests" / "fixtures" / "synthetic" / "regulatory"
...
def _load_fixture(submission_id: str) -> dict:
    path = FIXTURES_DIR / f"{submission_id}.json"
    if not path.exists():
        raise ToolError("EVIDENCE_ID_NOT_IN_RUN", f"No fixture for submission_id {submission_id!r}.")
    return json.loads(path.read_text())


def completeness_check(run_id: str, submission_id: str, evidence_ids: list[str], policy_contract_version: str) -> dict:
    """Matches regulatory_completeness_check.schema.json's input/output shape exactly."""
    if not run_id or not submission_id or not evidence_ids or not policy_contract_version:
    ...
    fixture = _load_fixture(submission_id)
