# Adaptation Notes — V1 Prompts → V2 Agentic Prompts

## Assessment

All 13 V1 prompts (`01_discovery.md` … `13_solution_proposal.md`) are **relevant and
...
`06_c4.md` already had explicit RAG/agent/HITL sections, since V1's governed workflows
...
1. **Output paths (all 13 files)** — every "Write under `participant-outputs-v2/XX-name/`
   ...
   `workshop/participant-output/` (this repo's equivalent of V1's `submission/artefacts/`).
   V1's `specs/` and `tasks/` top-level folders don't exist here; they map onto
...
2. **`04_ddd.md` §10 (agent responsibilities)** — added a requirement to name each agent
   explicitly and flag bounded-context crossings at agent handoffs, since V2 is genuinely
   multi-agent (V1's language here was written generically enough to not need rewriting,
   just sharpening).
...
3. **`06_c4.md`** — added a required "Agentic runtime view" cross-cutting section: the
   ...
   and LangSmith/OTel observability container. V1's "Gen AI runtime sketch" bullet stayed
...
4. **`08_technical_design.md`** — added §I2 "Agent/tool contracts": LangGraph state schema,
   ...
   V1 had no multi-agent runtime so had nothing to contract here.
...
5. **`12_assurance.md`** — added an "Agentic assurance" section: eval-ai-cache harness
   ...
   check, compliance evidence check. V1's assurance prompt covered design fidelity/data
   authority/AI quality/operability/control generically; this adds the agent-specific
   verification V2 needs.

## What's new (no V1 equivalent)

Prompts `14`–`23` — see `PROMPT_LIBRARY.md` V2 addendum table. These cover exactly the
capabilities V1 never needed because it wasn't a multi-agent, tool-using, cached,
