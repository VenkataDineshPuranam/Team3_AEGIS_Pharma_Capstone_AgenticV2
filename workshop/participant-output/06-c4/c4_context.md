...
**AEGIS-PHARMA Agentic Decision-Support System (V2)** — a governed, multi-agent system
...
Drawn directly from V1's `case/SOURCE_SYSTEM_FACT_PACK.md` (a real, if fictional, brownfield
...
| Existing AI platform (gateway, model endpoints, vector store, tools, evaluator — per `case/SOURCE_SYSTEM_FACT_PACK.md`'s "AI platform" domain row, noted as having "bundled vendor, stale entitlements, mutable manifests and weak cost controls") | Cross-cutting | **Not reused** — V2 is a deliberate replacement of this landscape's AI-platform row, not an integration with it (this session's explicit build-separate decision) |
| LLM provider (Anthropic Claude API) | All three core workflows | Outbound call — V2 depends on it; must have a documented degraded/AI-disabled path if unavailable (inherited V1 non-negotiable) |
...
**No external system in this list has a write integration.** Every arrow into V2 is
read-only evidence flow; every arrow out of V2 to these systems does not exist — this is the
...
[LLM provider (Anthropic)] ◀──────outbound calls──────▶│  Decision-Support System (V2)   │
