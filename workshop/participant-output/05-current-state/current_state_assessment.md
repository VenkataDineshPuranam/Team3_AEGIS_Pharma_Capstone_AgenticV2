...
**Executes:** `prompts/01_discovery.md`, applied reflexively to this V2 repository itself.
**DMAIC focus:** Measure.
...
| `prompts/` | 25 | **Real** — 13 adapted V1 prompts + 10 new V2 prompts (14–23) + library/adaptation notes |
...
3. **`knowledge/`, `evaluation/`, `runbooks/` hold README stubs only** (NAB-3, open). Whether
   V1's 32 knowledge docs and evaluation fixtures should be copied locally or kept as
...
- **NAB-1 (closed).** `knowledge/`, `evaluation/`, `runbooks/` had zero tracked files despite
  `README.md` claiming they were carried forward from V1 — a documented-vs-actual
  contradiction. Seeded with README stubs stating their purpose and current
  reference-not-copy status.
- **The "verify V1 grader behavior" action item (closed).** Raised at Stage 03, executed at
...
| 5 | Whether V1's knowledge/eval content should be copied locally | **Question** — open (NAB-3) |
...
| NAB-3 | Decide whether to copy V1's `knowledge/`/`evaluation/` content locally | Stage 13 (ontology needs domain knowledge), Stage 14 (eval floor) | Medium |
