...
|---|---|---|
| **Context of the organization / interested parties** (4) | `docs/product/discovery/discovery.md`, `case/STAKEHOLDER_PACK.md` (V1, verified) | N/A — foundational/documentary clause, no runtime evidence expected |
| **Leadership / roles & responsibilities** (5) | `hitl_control_model.md` §3 (bounded-context owners), `control_ownership.md` (Stage 16, per-policy owners) | Roles are real (verbatim from V1's stakeholder pack), not invented — `hitl_control_model.md` §1 |
...
| **Incident management** | `evidence/incidents/README.md` (empty scaffold) | **Gap G-3** — no incident-response procedure exists yet; `AI_INCIDENT_RESPONSE.md` (K-004) is a real V1 knowledge doc the system can *cite* but the *organization's own* incident procedure for this system doesn't exist |
