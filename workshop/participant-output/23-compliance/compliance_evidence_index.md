...
| Traces are actually captured (Stage 17 design → real operation) | LangSmith project `aegis-pharma-v2` — real run IDs queried via the LangSmith API this session (e.g. `019ff9ed-...` runs for `finalize`, `hitl_interrupt`, `route_after_hitl`) | `smith.langchain.com`, project `aegis-pharma-v2`, or the LangSmith API |
