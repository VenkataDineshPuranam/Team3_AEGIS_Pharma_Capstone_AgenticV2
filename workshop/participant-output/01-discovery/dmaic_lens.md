...
The problem this stage resolves: establish, from evidence, what carries forward from V1
...
Baselines available from V1, directly countable this pass:
...
- Baselines that are **Unknown** (must not be estimated): V2 token cost per workflow run,
  V2 agent count, V2 latency per workflow, V1's actual eval scorecard results (never read
  the numbers in `submission/evaluation/reports/scorecard.csv` this pass), V1's rubric
  point totals achieved by the prior completed run.

## Analyze

Root causes visible from Discovery evidence alone:
- V1's single-shot architecture is a root cause of the exact gap the user is asking V2 to
  ...
  measured fact.
- The tension between V1's "offline-compatible" framing and a LangSmith-based observability
...
**provisional**:
- Preserve V1's domain model, constraints, and eval floor unchanged; treat the agentic
...
- Revisit trigger: if EAB-2 (offline/hosted tension) resolves toward "hosted-only," several
  V1 non-negotiables framed around offline-compatibility should be explicitly revised, not
  silently dropped.
