# DOWNTIME Waste Register — Stage 03 (C4), updated from DDD

**C4-stage update:** carried forward from `docs/architecture/ddd/waste_register_downtime.md`.
One row refined with an architecture-level finding (see `dmaic_lens.md` Measure/Analyze):
the **Waiting** row now has a concrete architectural mitigation for the one part of it that
*was* addressable — Agent Workers as a container distinct from Orchestrator API, so a
long-running Supply Planning search doesn't force the synchronous path to wait. The HITL
wait itself remains classified business-required NVA, unchanged. No rows added or removed
at C4 level; all Stage 01/02 findings still hold.
