...
| EAB-3 | Determine if a V3-specific case/stakeholder-pack addendum is needed (agent authority ownership, HITL role definitions) | User / domain SME | DDD (Stage 06) needs an owner for each agent's authority limit; governance (Stage 16) needs a named HITL approver role | 06, 16 | Blocks design |
