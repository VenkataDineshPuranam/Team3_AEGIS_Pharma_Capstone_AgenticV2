# AI-Specific Waste Register — Stage 04 (ADR), **closed out**

**ADR-stage close-out.** Each of the 8 categories now has a named treatment decision.

| Category | Treatment decision | Owning ADR / deferral |
|---|---|---|
| **Token** | Per-graph token/step budgets; no cross-graph agent chains multiplying calls | ADR-008; **numeric budgets deferred to Stage 08 §I2 / Stage 15** (deliberately — setting numbers now would be guessing, per EAB-6) |
| **Retrieval** | Per-bounded-context scoping; `untrusted`/`superseded` filtered at the retrieval boundary before ranking | ADR-003 |
| **Model** | Authority and unit/identity checks are deterministic lookups, never model judgment — removes the "ask the LLM to verify trustworthiness" anti-pattern | ADR-003 |
| **Human-review** | 100% HITL routing for PV/Supply accepted as a regulatory requirement, not waste to optimize away; Batch Review's self-resolution path is governed rather than unrestricted | ADR-004; risk-tiering detail deferred to Stage 16 |
| **Evaluation** | Correctness gates must pass under every dependency-failure mode | ADR-007; fast-subset vs full-regression split **deferred to Stage 14** |
| **Integration** | ADR-005 knowingly accepts added integration surface (separate Policy Engine) in exchange for removing policy-drift Defects risk; fails closed | ADR-005 |
| **Context** | LangGraph shared state schema is the single source of truth; no per-agent context reconstruction | ADR-001; schema detail at Stage 08 §I2 |
| **Observability** | Owned audit store separate from hosted LangSmith, so compliance evidence isn't coupled to a vendor SLA; both sinks redacted before write | ADR-006 |

All Stage 01–03 findings still hold; no categories added or removed. Three items are
explicitly **deferred with a named stage** rather than resolved here — recorded so they
surface at Stage 09 consolidation rather than being lost.
