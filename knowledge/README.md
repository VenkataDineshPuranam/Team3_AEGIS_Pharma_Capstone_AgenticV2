...
V1's `knowledge/` (32 pharma/GxP/PV/AI-governance policy documents) plus
...
independent of the V1 checkout being present on disk.

**Why this doesn't reopen ADR-002.** ADR-002 says V2 is built separately from V1 with **no
code reuse** — a decision about not inheriting V1's *application architecture*. This corpus is
...
used as poisoned-evidence test cases). Copying it is using V1 as the evidence source it was
always meant to be, not reusing V1's implementation.

**Scope boundary — NAB-3 only half-resolved.** This copy covers `knowledge/` only. V1's
