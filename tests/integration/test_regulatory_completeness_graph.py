"""Stage 21 integration tests -- regulatory_completeness graph shape, routing, guards, HITL
...
def _run_to_interrupt(submission_id: str):
    run_id = f"R-regulatory-{uuid.uuid4().hex[:8]}"
    graph = build_regulatory_graph(submission_id=submission_id)
    config = {"configurable": {"thread_id": run_id}}
    state = new_state(run_id=run_id, workflow="regulatory_completeness", requester_role="Head of Regulatory Affairs")
    result = graph.invoke(state, config=config)
    return graph, config, result


def test_clean_regulatory_reaches_hitl_interrupt_and_approves_to_completed():
    graph, config, result = _run_to_interrupt("REG-001")
...
def test_idmp_identity_conflict_still_reaches_hitl():
    """INJ-045: IDMP identity conflict surfaces as a conflict finding, exercised end to
    end through the real graph."""
    graph, config, result = _run_to_interrupt("REG-002")
    assert "__interrupt__" in result
...
def test_prohibited_submission_ready_term_is_blocked():
    """ADR-004 layer 3: the guard blocks output asserting submission readiness -- the
    ...
    contract = get_prohibition_contract("v1", "regulatory_completeness")
    assert isinstance(contract, ProhibitionContract)
    draft = DecisionSupportOutput(
        summary="Submission is ready for filing.",
        claims=(Claim(text="Submission is ready for filing.", cites=("K-025",)),),
    )
...
def test_policy_engine_unreachable_refuses_closed(monkeypatch):
    import services.api.regulatory_graph as regulatory_module

    def _raise(*args, **kwargs):
        from services.integration.policy_engine import PolicyEngineUnavailable
        raise PolicyEngineUnavailable("simulated outage")

    monkeypatch.setattr(regulatory_module, "get_prohibition_contract", _raise)
    run_id = f"R-regulatory-{uuid.uuid4().hex[:8]}"
    graph = regulatory_module.build_regulatory_graph(submission_id="REG-001")
    state = new_state(run_id=run_id, workflow="regulatory_completeness", requester_role="Head of Regulatory Affairs")
